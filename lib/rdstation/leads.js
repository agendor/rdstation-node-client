...
function Leads(token){
	...
	this.chageStatusToLead = function chageStatusClient(email, opportunity ) {
		return chageStatus(email, opportunity, 0)
	};

	this.chageStatusToQualificado = function chageStatusClient(email, opportunity ) {
		return chageStatus(email, opportunity, 1)
	};
	
	this.chageStatusToClient = function chageStatusClient(email, opportunity ) {
